...
--Solution No:-2
select name from Artist where ArtistId in (select ArtistId from Album group by ArtistId having
(count(ArtistId)= (select MIN(ArtistId) from Album where ArtistId in (select count(ArtistId) from Album group by ArtistId)) ) )



--Solution No:-3
select count(name) as 'Number of Artist' from Artist where ArtistId in (select ArtistId from Album group by ArtistId having
(count(ArtistId)= (select MIN(ArtistId) from Album where ArtistId in (select count(ArtistId) from Album group by ArtistId)) ) )


--Solution No:-4
select name from Artist where ArtistId in(select ArtistId from Album where AlbumId in(select AlbumId from Song where Duration >5.00)  )



--Solution No:-5
select name from Artist where ArtistId in(select count(ArtistId),name from Album where AlbumId in(select AlbumId from Song where Duration <5.00) group by ArtistId,name )

select count(ArtistId) from Album where AlbumId in(select AlbumId from Song where Duration <5.00) group by ArtistId



--Solution No:-6
--select YEAR from Album group by year having COUNT(Year)=2 

--select * from Album
--select * from Song
select year, COUNT(year) from Album group by year having COUNT(Year) >1
--select AlbumId from Album where Year in (select COUNT(year) as ' Number of Same year of different album' from Album group by year)

select year from Album where Year  in (select year from Album group by year  having COUNT(Year) >1 ) group by year
...
select SUM(duration) as 'Total Duration' ,AlbumId from Song    group by AlbumId order by [Total Duration]  desc
